...
<div class="container">
    ...
    {{-- Guardado visual por policy --}}
    @cannot('create', App\Models\CuentaCobro::class)
        <div class="alert alert-danger">
            <i class="fas fa-ban me-2"></i>No tienes permisos para crear cuentas de cobro.
        </div>
    @endcannot
    ...
    <form action="{{ route('cuenta.cobro.guardar') }}" method="POST" id="formCrearCuenta" enctype="multipart/form-data">
        ...
        <div class="form-section">
            <h5><i class="fas fa-user-tie"></i> Datos de quien cobra</h5>
            <div class="row g-3">
                <div class="col-md-6">
                    <label for="nombre_cobrador" class="form-label required">
                        <i class="fas fa-id-card"></i>Nombre completo
                    </label>
                    <input type="text" name="nombre_cobrador" id="nombre_cobrador"
                           class="form-control @error('nombre_cobrador') is-invalid @enderror"
                           value="{{ old('nombre_cobrador') }}" placeholder="Ej: Juan Pérez García" required>
                    @error('nombre_cobrador') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label for="documento_cobrador" class="form-label required">
                        <i class="fas fa-fingerprint"></i>Número de documento
                    </label>
                    <input type="text" name="documento_cobrador" id="documento_cobrador"
                           class="form-control @error('documento_cobrador') is-invalid @enderror"
                           value="{{ old('documento_cobrador') }}" placeholder="Ej: 1234567890" required>
                    @error('documento_cobrador') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12">
                    <label for="direccion_cobrador" class="form-label required">
                        <i class="fas fa-map-marker-alt"></i>Dirección
                    </label>
                    <input type="text" name="direccion_cobrador" id="direccion_cobrador"
                           class="form-control @error('direccion_cobrador') is-invalid @enderror"
                           value="{{ old('direccion_cobrador') }}" placeholder="Ej: Calle 123 #45-67" required>
                    @error('direccion_cobrador') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label for="telefono_cobrador" class="form-label required">
                        <i class="fas fa-phone"></i>Teléfono
                    </label>
                    <input type="text" name="telefono_cobrador" id="telefono_cobrador"
                           class="form-control @error('telefono_cobrador') is-invalid @enderror"
                           value="{{ old('telefono_cobrador') }}" placeholder="Ej: 3001234567" required>
                    @error('telefono_cobrador') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label for="email_cobrador" class="form-label required">
                        <i class="fas fa-envelope"></i>Correo electrónico
                    </label>
                    <input type="email" name="email_cobrador" id="email_cobrador"
                           class="form-control @error('email_cobrador') is-invalid @enderror"
                           value="{{ old('email_cobrador') }}" placeholder="ejemplo@correo.com" required>
                    @error('email_cobrador') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
            </div>
        </div>
        ...
        {{-- Datos del cliente --}}
        <div class="form-section">
            <h5><i class="fas fa-building"></i> Datos del cliente</h5>
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="nombre_cliente" id="nombre_cliente"
                           ...
                           value="{{ old('nombre_cliente') }}" placeholder="Ej: Empresa ABC S.A.S." required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="documento_cliente" id="documento_cliente"
                           ...
                           value="{{ old('documento_cliente') }}" placeholder="Ej: 900123456-7" required>
                    @error('documento_cliente') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            <h5><i class="fas fa-file-invoice"></i> Detalles de la Cuenta</h5>
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="number" step="0.01" min="1" name="monto" id="monto"
                           ...
                           value="{{ old('monto') }}" placeholder="Ej: 1000000.00" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (documentoCobradorInput) {
        documentoCobradorInput.addEventListener('input', function() {
            this.value = this.value.replace(/[^0-9]/g, '');
        });
    }
